$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = '7599a1127f7561cd092e4e63c9e8a3d5';
    var title = $('.movie').val();
    if (title=="") {
...
function movie_recs(movie_title,movie_id,my_api_key){
  $.ajax({
    ...
    success: function(recs){
      if(recs=="Whoops! Our movie elves couldnt find it. Maybe its hiding behind the popcorn machine? Try again—or offer them nachos!"){
        $('.fail').css('display','block');
        $('.results').css('display','none');
        $("#loader").delay(500).fadeOut();
      }
